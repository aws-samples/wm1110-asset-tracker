...
class ProvisionWrapper:
    ...
    def generate_mfg(self, output_dir, input_type, wireless_device_path=None, device_profile_path=None,
                     certificate_json=None, address=None):
    ...
    def generate_bin_and_hex_from_aws_jsons(self, device_json, profile_json, board, out_bin, chip, out_hex, addr):
        ...
        args = [sys.executable, 'provision.py', platform, 'aws', '--wireless_device_json', device_json,
                                      ...
                                      '--output_bin', out_bin, '--chip', chip, '--output_hex', out_hex, "--addr", addr]
    ...
    def generate_bin_and_hex_from_certificate_json(self, certificate, board, out_bin, chip, out_hex, addr):
        assert board in (BoardType.Nordic, BoardType.TI), "Operation supported only for Nordic and TI"
